...
class FileHandler {
  ...
  async getAll() {
    try {
      ...
      if (exist) {
        return await fs.promises.readFile(`./data/db/filesystem/${this.fileName}.txt`, 'utf8');
      }
  ...
  async saveProduct(prod) {
    try {
      ...
      if(exist) {
        let fileData = await fs.promises.readFile(`./data/db/filesystem/${this.fileName}.txt`, 'utf8');
        ...
        data.push(prod);
        await fs.promises.writeFile(`./data/db/filesystem/${this.fileName}.txt`, JSON.stringify(data, null, 2));
  ...
  async exist() {
    try {
      let exist = false;
      let files = await fs.promises.readdir('./data/db/filesystem/');
  ...
  async saveUpdate(products) {
    try {
      let exist = await this.exist().then(data => data);
      if (exist) {
        await fs.promises.writeFile(`./data/db/filesystem/${this.fileName}.txt`, JSON.stringify(products, null, 2));
  ...
  async createCart() {
    try {
      ...
      cart.id = 1;
      await fs.promises.writeFile(`./data/db/filesystem/${this.fileName}.txt`, JSON.stringify(cart, null, 2));
      return `Carrito con id ${cart.id} creado.`;
    } catch (err) {
      return console.log(err);
    }
  ...
  async deleteCart(id) {
    try {
      ...
      if (exist) {
        await fs.promises.unlink(`./datadb/filesystem//${this.fileName}.txt`);
